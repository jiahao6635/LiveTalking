...
# Import SSL patch to bypass certificate verification
...
###########################################################################################
class BailianCosyVoiceTTS(BaseTTS):
    """百炼平台 CosyVoice 云服务 - 使用官方 SDK"""
    def __init__(self, opt, parent):
        ...
        voice = opt.REF_TEXT if opt.REF_TEXT else "longxiaochun"
        if voice and not voice.endswith('_v2'):
            self.voice = f"{voice}_v2"
        else:
            self.voice = voice
        logger.info(f"百炼CosyVoice初始化: model={self.model}, voice={self.voice}")

    def txt_to_audio(self, msg: tuple[str, dict]):
        text, textevent = msg
        try:
            self.stream_synthesis(text, msg)
        except Exception as e:
            logger.exception('百炼CosyVoice合成失败')

    def stream_synthesis(self, text: str, msg: tuple[str, dict]):
        text_content, textevent = msg
        audio_queue = Queue()
        synthesis_complete = Event()
        synthesis_error = [None]

        class StreamCallback(self.ResultCallback):
            def __init__(self, queue, complete_event, error_container, parent_tts):
                self.queue = queue
                self.complete_event = complete_event
                self.error_container = error_container
                self.parent_tts = parent_tts

            def on_open(self):
                logger.info("百炼CosyVoice连接已建立")

            def on_complete(self):
                self.complete_event.set()

            def on_error(self, message: str):
                logger.error(f"百炼CosyVoice错误: {message}")
                self.error_container[0] = message
                self.complete_event.set()
            ...
            def on_data(self, data: bytes) -> None:
                if data and self.parent_tts.state == State.RUNNING:
                    self.queue.put(data)

        try:
            callback = StreamCallback(audio_queue, synthesis_complete, synthesis_error, self)
            synthesizer = self.SpeechSynthesizer(
                model=self.model,
                voice=self.voice,
                format=self.AudioFormat.PCM_22050HZ_MONO_16BIT,
                callback=callback
            )
            ...
            first_audio = True
            while not synthesis_complete.is_set() or not audio_queue.empty():
                try:
                    ...
                    while streamlen >= self.chunk and self.state == State.RUNNING:
                        eventpoint = {}
                        if first_audio:
                            eventpoint = {'status': 'start', 'text': text_content}
                            eventpoint.update(**textevent)
                            first_audio = False
                        self.parent.put_audio_frame(stream[idx:idx+self.chunk], eventpoint)
                        streamlen -= self.chunk
            ...
            eventpoint = {'status': 'end', 'text': text_content}
            eventpoint.update(**textevent)
            self.parent.put_audio_frame(np.zeros(self.chunk, np.float32), eventpoint)
        except Exception as e:
            logger.exception('百炼CosyVoice异常')
